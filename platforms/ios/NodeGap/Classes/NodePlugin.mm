...
@implementation NodePlugin

static NodePlugin* g_loop_self = nil;
static CDVInvokedUrlCommand* g_loop_command = nil;

static void _stdout_write(const char* str)
{
	if (g_loop_self && g_loop_command)
	{
		NSString* message = [NSString stringWithFormat: @"%s", str];
		CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:message];
		[pluginResult setKeepCallbackAsBool:YES];
		[g_loop_self.commandDelegate sendPluginResult:pluginResult callbackId:g_loop_command.callbackId];
	}
}
...
- (void) loop: (CDVInvokedUrlCommand*) command
{
	...
	{
		g_loop_self = self;
		g_loop_command = command;

		int err = node_gap_loop(_stdout_write, _stderr_write);

		CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:err];
		[pluginResult setKeepCallbackAsBool:YES];
...
- (void) exec: (CDVInvokedUrlCommand*) command
{
	[self.commandDelegate runInBackground: ^
	{
		...
		{
			const char* node_script = [script UTF8String];
